...
public class CreditAccountServiceImpl implements CreditAccountService {
  ...
  public void checkAllCredits() {
    ...
    creditRepository.findAllByStatus(CreditStatus.OVERDUE)
        .forEach(credit -> {
        // TODO: test this.
          String subject = "Credit overdue";
          String template = "credit-overdue.html";
          Map<String, Object> attributes = new HashMap<>();
          attributes.put("firstname", credit.getAccount().getOwner().getFirstname());
          mailSender.sendMessageHtml(credit.getAccount().getOwner().getMail(), subject, template, attributes);
          credit.addPenalty();
  ...
  public void checkAllCreditAccount() {
    accountRepository.findAllByStatus(AccountStatus.WAITING)
        .stream()
        .filter(account -> Date.valueOf(LocalDate.now().minusDays(DAYS_BEFORE_ACCOUNT_DECLINED))
            .compareTo(account.getLastTransactionDate()) == 0)
        .forEach(account -> {
  ...
  private boolean isYearSameAndMonthNext(Card card) {
    return Integer.parseInt(card.getYear()) == LocalDate.now().getYear()
        && Integer.parseInt(card.getMonth()) < LocalDate.now().getMonthValue();
  }
