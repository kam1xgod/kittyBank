...
public class TransferServiceImpl implements TransferService {
  ...
  // todo: transfers with sender number 0 - from bank. like capitalization and
  // etc.
  ...
  public List<Transfer> commitTransfer(Transfer transfer) {
    try {
      if (checkTransfer(transfer) && !fraudMonitor.checkTransfer(transfer)) {
        ...
        if (!Objects.isNull(getRecipientIfInDatabase(transfer.getRecipient().getNumber()))) {
  ...
  private Account getAccountFromCardOrAccount(String cardOrAccountNumber) {
    ...
    if (card != null) {
      ...
      account = accountRepository.findByNumber(cardOrAccountNumber)
          .orElseThrow(
              () -> new ApiRequestException("There is no card or account with this number.", HttpStatus.BAD_REQUEST));
    }
    return account;
  }
  ...
  private String getCardNumberFromAccountId(Long accountId) {
    Account account = accountRepository.findById(accountId)
        .orElseThrow(() -> new ApiRequestException("This account doesn't exist.", HttpStatus.BAD_REQUEST));
  ...
  private Account getRecipientIfInDatabase(String recipientAccountNumber) {
    return accountRepository.findByNumber(recipientAccountNumber).orElse(null);
  }
}
