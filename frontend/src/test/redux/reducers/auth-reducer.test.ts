...
test("Login Success", () => {
    const state: InitialStateType = authReducer(store.getState().auth, loginSuccess(role));
...
test("Activate Account Success", () => {
    const state: InitialStateType = authReducer(store.getState().auth, activateUserSuccess(success));
    expect(state.success).toEqual(success);
});

test("Activate Account Failure", () => {
    const state: InitialStateType = authReducer(store.getState().auth, activateUserFailure(error));
    expect(state.error).toEqual(error);
});
